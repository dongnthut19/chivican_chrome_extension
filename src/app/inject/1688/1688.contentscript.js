function Worker1688() {
  ...
  function iconAddProductForNewUI($button) {
    setTimeout(() => {
      ...
      // ảnh
      const newImageProp = $(".pc-sku-wrapper .prop-item-wrapper .prop-item .active .prop-img");
      console.log("🚀 ~ file: 1688.contentscript.js ~ line 417 ~ setTimeout ~ newImagePro", newImageProp);
      
      let productImage = "";
      if (newImageProp.length > 0) {
        const rawProductImage = newImageProp[0].attributes[1].value;
        productImage = rawProductImage.replace('background: url("', '').replace('") center center / contain no-repeat; width: 36px; height: 36px; margin-right: 2px;', '');
      } else if (parentNode.children.length === 3) { // sku có ảnh
        const rawProductImage = parentNode.children[0].attributes[1].nodeValue;
        productImage = rawProductImage.replace('background: url("', '').replace('") center center / contain no-repeat; width: 36px; height: 36px; margin-right: 10px;', '');
      } else {
        const productImgs = $(
          ".img-list-wrapper .detail-gallery-turn-wrapper img.detail-gallery-img"
        );
